...
export function SettingsSidebar({ visible, onHide }: Props) {
  ...
  return (
    <Sidebar visible={visible} onHide={onHide} position="right" header={t('settings.title')}>
      <div className="flex flex-column h-full">
        {/* Scrollable settings region */}
        <div className="flex flex-column gap-4 flex-1 overflow-y-auto">
          {/* Mode selector */}
          <div>
        ...
        {/* Pinned danger footer */}
        <div className="pt-4 mt-4 border-top-1 surface-border">
          <Button
            label={t('settings.purge.button')}
            icon="pi pi-trash"
            severity="danger"
            outlined
            className="w-full"
            onClick={() => setPurgeDialogVisible(true)}
          />
        </div>

        <PurgeDataDialog visible={purgeDialogVisible} onHide={() => setPurgeDialogVisible(false)} />
      </div>
    </Sidebar>
  )
}
